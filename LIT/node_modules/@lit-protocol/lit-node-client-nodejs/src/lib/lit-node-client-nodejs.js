...
class LitNodeClientNodeJs extends core_1.LitCore {
        ...
        this.getSessionKey = () => {
            ...
            if (storedSessionKeyOrError.type === "ERROR" /* EITHER_TYPE.ERROR */ ||
                ...
                try {
                    ...
                    (0, misc_1.log)(`[getSessionKey] Localstorage not available.Not a problem.Contiune...`);
                }
    ...
    `);
        ...
        this.checkNeedToResignSessionKey = async ({ authSig, sessionKeyUri, resourceAbilityRequests, }) => {
            ...
            if (authSig.algo === `ed25519` || authSig.algo === undefined) {
                try {
                    ...
                    (0, misc_1.log)(`Error while verifying ECDSA signature: `, e);
                    return true;
                }
            }
            else if (authSig.algo === `LIT_BLS`) {
                try {
                    (0, validate_bls_session_sig_1.blsSessionSigVerify)(blsSdk.verify_signature, this.networkPubKey, authSig);
                }
                catch (e) {
                ...
                (0, misc_1.throwError)({
                    ...
                    errorKind: constants_1.LIT_ERROR.SIGNATURE_VALIDATION_ERROR.kind,
                    errorCode: constants_1.LIT_ERROR.SIGNATURE_VALIDATION_ERROR.code,
                });
            }
        ...
        this.getSessionSignatures = (signedData) => {
            ...
            keys.forEach((key) => {
                ...
                const sigShares = shares.map((s, index) => {
                    ...
                    const share = (0, get_signatures_1.getFlattenShare)(s);
        ...
        this.executeJs = async (params) => {
            ...
            const nodePromises = this.getNodePromises(async (url) => {
                // -- choose the right signature
                const sessionSig = this.getSessionSigByUrl({
            ...
            const signatures = (0, get_signatures_1.getSignatures)({
                requestId,
        ...
        this.pkpSign = async (params) => {
            ...
            const res = await this.handleNodePromises(nodePromises, requestId, this.connectedNodes.size // ECDSA requires responses from all nodes, but only shares from minNodeCount.
            );
            // ========== Handle Response ==========
            // -- case: promises rejected
            if (!res.success) {
        ...
        this.getSignedToken = async (params) => {
            ...
            const nodePromises = this.getNodePromises((url) => {
                ...
                const urlWithPath = (0, core_1.composeLitUrl)({
                    url,
                    endpoint: constants_1.LIT_ENDPOINT.SIGN_ACCS,
                });
                return this.generatePromise(urlWithPath, reqBody, requestId);
            });
            // -- resolve promises
        ...
        this.signSessionKey = async (params) => {
            ...
            if (!sessionKeyUri) {
                throw new Error('[signSessionKey] sessionKeyUri is not defined. Please provide a sessionKeyUri or a sessionKey.');
            }
            ...
            if (!curveType) {
                (0, misc_1.log)(`[signSessionKey] curveType not found. Defaulting to ECDSA.`);
                curveType = 'ECDSA';
            }
            ...
            if (validatedSignedDataList.length < this.config.minNodeCount) {
                throw new Error(`[signSessionKey] not enough nodes signed the session key.  Expected ${this.config.minNodeCount}, got ${validatedSignedDataList.length}`);
            }
            ...
            const blsCombinedSignature = blsSdk.combine_signature_shares(signatureShares.map((s) => JSON.stringify(s)));
      ...
      const resourceAbilityRequests = [
            ...
            const pkpSessionSigs = this.getSessionSigs({
                ...
                authNeededCallback: async (props) => {
                    ...
                    if (props.litActionCode && props.litActionIpfsId) {
                        throw new Error('[getPkpSessionSigs/callback]litActionCode and litActionIpfsId cannot exist at the same time');
                    }
                    /**
        ...
        this.getLitActionSessionSigs = async (params) => {
            ...
            if (!params.litActionCode && !params.litActionIpfsId) {
                throw new Error("Either 'litActionCode' or 'litActionIpfsId' must be provided.");
            }
