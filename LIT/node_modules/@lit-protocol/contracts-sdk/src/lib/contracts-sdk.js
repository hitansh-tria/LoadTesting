...
class LitContracts {
  ...
  // ***********************************************************************************************
        `);
            ...
            const addresses = await LitContracts.getContractAddresses(this.network, this.customContext?.provider ?? this.provider, this.customContext);
         ...
         */
        this.mintWithAuth = async ({ authMethod, scopes, pubkey, authMethodId, }) => {
            // -- validate
            if (!this.connected) {
...
https://developer.litprotocol.com/v3/sdk/wallets/auth-methods/#auth-method-scopes
      `);
            ...
            // -- start minting
            const tx = await this.pkpHelperContract.write.mintNextAndAddAuthMethods(2, // key type
            [authMethod.authMethodType], [_authMethodId], [_pubkey], [[...scopes]], true, true, {
                value: mintCost,
            });
            ...
            if (!events[0].topics || events[0].topics.length < 1) {
                throw new Error(`No topics found in events, cannot derive pkp information. Transaction hash: ${receipt.transactionHash} If you are using your own contracts please use ethers directly`);
            }
        ...
        */
        ...
        this.addPermittedAuthMethod = async ({ pkpTokenId, authMethodType, authMethodId, authMethodScopes, webAuthnPubkey, }) => {
            ...
            try {
                const res = await this.pkpPermissionsContract.write.addPermittedAuthMethod(pkpTokenId, {
        ...
        this.addPermittedAction = async ({ ipfsId, pkpTokenId, authMethodScopes, }) => {
            ...
            try {
                const res = await this.pkpPermissionsContract.write.addPermittedAction(pkpTokenId, ipfsIdBytes, scopes);
                const receipt = await res.wait();
                return receipt;
            }
            catch (e) {
         ...
         */
        this.mintCapacityCreditsNFT = async ({ requestsPerDay, requestsPerSecond, requestsPerKilosecond, daysUntilUTCMidnightExpiration, }) => {
            ...
            if ((requestsPerDay === null ||
                ...
                throw new Error('At least one of requestsPerDay, requestsPerSecond, or requestsPerKilosecond is required and must be more than 0');
            }
            ...
            read: {
                ...
                getTokensInfoByAddress: async (ownerAddress) => {
                    ...
                    for (let i = 0; i < tokenIds.length; i++) {
                        ...
                        const addrs = await (0, addresses_1.derivedAddresses)({
                            pkpTokenId: tokenId,
                            publicKey: pubKey,
                        });
                        arr.push(addrs);
            ...
            write: {
                mint: async () => {
                    ...
                    if (this.isPKP) {
                        ...
                        const tx = await this.pkpNftContract.write.populateTransaction.mintNext(2, {
                            value: mintCost,
                        });
                        this.log('tx:', tx);
                        this.log('...signing tx');
                ...
                claimAndMint: async (derivedKeyId, signatures, txOpts) => {
                    ...
                    const tx = await this.pkpNftContract.write.claimAndMint(2, derivedKeyId, signatures, txOpts ?? { value: cost });
        ...
        this.pkpPermissionsContractUtils = {
            ...
            write: {
                ...
                addPermittedAction: async (pkpId, ipfsId) => {
                    ...
                    const tx = await this.pkpPermissionsContract.write.addPermittedAction(tokenId, ipfsIdBytes, [1]);
                ...
                addPermittedAddress: async (pkpId, ownerAddress) => {
                    ...
                    this.log('[addPermittedAddress] input<pkpId>:', pkpId);
                    const tx = await this.pkpPermissionsContract.write.addPermittedAddress(pkpId, ownerAddress, [1]);
        ...
        this.rateLimitNftContractUtils = {
            ...
            write: {
                ...
                transfer: async ({ fromAddress, toAddress, RLITokenAddress, }) => {
                    ...
                    const tx = await this.rateLimitNftContract.write.transferFrom(fromAddress, toAddress, RLITokenAddress);
                    this.log('tx:', tx);
                    // const res = await tx.wait();
                    // return {
                    //     tx,
        ...
        this.pkpHelperContractUtil = {
            ...
            write: {
                 ...
                 * @returns
                 */
                mintNextAndAddAuthMethods: async ({ keyType, permittedAuthMethodTypes, permittedAuthMethodIds, permittedAuthMethodPubkeys, permittedAuthMethodScopes, addPkpEthAddressAsPermittedAddress, sendPkpToItself, }) => {
                    // first get mint cost
                    const mintCost = await this.pkpNftContract.read.mintCost();
                    const tx = await this.pkpHelperContract.write.mintNextAndAddAuthMethods(keyType, permittedAuthMethodTypes, permittedAuthMethodIds, permittedAuthMethodPubkeys, permittedAuthMethodScopes, addPkpEthAddressAsPermittedAddress, sendPkpToItself, { value: mintCost });
                    return tx;
                },
    ...
    static async getStakingContract(network, context, rpcUrl) {
        ...
        if (!context) {
            const contractData = await LitContracts._resolveContractContext(network, context);
            ...
            if (!context.resolverAddress) {
                ...
                const contractContext = await LitContracts._getContractsFromResolver(context, provider, ['Staking']);
                if (!contractContext.Staking.address) {
                    throw new Error('❌ Could not get Staking Contract from contract resolver instance');
                }
                return new ethers_1.ethers.Contract(contractContext.Staking.address, contractContext.Staking.abi ?? StakingData_1.StakingData.abi, provider);
            }
        }
    }
    ...
    static async getContractAddresses(network, provider, context) {
        ...
        if (context) {
            ...
            if (context?.resolverAddress) {
                context = await LitContracts._getContractsFromResolver(context, provider);
            }
            const flatten = [];
    ...
    static async _resolveContractContext(network, context) {
        ...
        if (network === 'cayenne') {
            ...
            try {
                ...
                throw new Error(`Error fetching data from ${MANZANO_API}: ${e.toString()}`);
            }
        }
            ...
            try {
                ...
                throw new Error(`Error fetching data from ${HABANERO_API}: ${e.toString()}`);
            }
        }
...
LitContracts.getValidators = async (network, context, rpcUrl) => {
    const contract = await LitContracts.getStakingContract(network, context, rpcUrl);
    // Fetch contract data
    ...
    const networks = activeValidatorStructs.map((item) => {
        ...
        return `${proto}${(0, hex2dec_1.intToIP)(item.ip)}:${item.port}`;
    });
    return networks;
};
