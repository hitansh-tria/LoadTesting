...
const EPOCH_PROPAGATION_DELAY = 30000;
...
class LitCore {
    ...
    constructor(config) {
        ...
        this._blockHashUrl = 'http://block-indexer.litgateway.com/get_most_recent_valid_block';
        // ========== Logger utilities ==========
        ...
        this.getLatestBlockhash = async () => {
            ...
            if (!this.latestBlockhash) {
                throw new Error(`latestBlockhash is not available. Received: "${this.latestBlockhash}"`);
            }
         ...
         */
        this.computeHDPubKey = (keyId, sigType = constants_1.LIT_CURVE.EcdsaCaitSith) => {
        ...
        if (!(config.litNetwork in constants_1.LIT_NETWORKS)) {
            return (0, misc_1.throwError)({
                message: 'Unsupported network has been provided please use a "litNetwork" option which is supported ("cayenne", "habanero", "manzano")',
                errorKind: constants_1.LIT_ERROR.INVALID_PARAM_TYPE.kind,
                errorCode: constants_1.LIT_ERROR.INVALID_PARAM_TYPE.code,
            });
        }
    ...
    async _getValidatorData() {
        const [minNodeCount, bootstrapUrls] = await Promise.all([
            contracts_sdk_1.LitContracts.getMinNodeCount(this.config.litNetwork, this.config.contractContext, this.config.rpcUrl),
            contracts_sdk_1.LitContracts.getValidators(this.config.litNetwork, this.config.contractContext, this.config.rpcUrl),
        ]);
        if (minNodeCount <= 0) {
            (0, misc_1.throwError)({
    ...
    _scheduleEpochNumberUpdate() {
        ...
        this._epochUpdateTimeout = setTimeout(async () => {
            try {
                ...
                (0, misc_1.logError)('Error while attempting to fetch current epoch number');
            }
        ...
        if (this._epochUpdateTimeout.unref &&
    ...
    async _handleStakingContractStateChange(state) {
        ...
        if (state === constants_1.StakingStates.Active) {
            ...
            if (this.config.litNetwork === constants_1.LitNetwork.Manzano ||
                ...
                try {
                    ...
                    const delta = newNodeUrls.filter((item) => existingNodeUrls.includes(item));
    ...
    async _connect() {
        ...
        if (!this.config.contractContext) {
            this.config.contractContext = await contracts_sdk_1.LitContracts.getContractAddresses(this.config.litNetwork, new ethers_1.ethers.providers.JsonRpcProvider(this.config.rpcUrl || constants_1.LIT_CHAINS['lit'].rpcUrls[0]));
        }
            ...
            throw new Error('The provided contractContext was missing the "Staking" contract`');
        }
        if (this.config.contractContext) {
            ...
            if (this.config.litNetwork === constants_1.LitNetwork.Custom) {
        ...
        const getStakingContract = this._stakingContract ||
            contracts_sdk_1.LitContracts.getStakingContract(this.config.litNetwork, this.config.contractContext, // We've already primed the `contractContext`
            this.config.rpcUrl);
        const [{ minNodeCount, bootstrapUrls }, stakingContract] = await Promise.all([this._getValidatorData(), getStakingContract]);
    ...
    async handshakeAndVerifyNodeAttestation({ url, requestId, }) {
    ...
    async _syncBlockhash() {
        ...
        return fetch(this._blockHashUrl)
            ...
            (0, misc_1.logError)('Error while attempting fetch new latestBlockhash:', err.messages, 'reason: ', err.reason);
        });
    }
    ...
    _scheduleNetworkSync() {
        ...
        this._networkSyncInterval = setInterval(async () => {
            if (!this.lastBlockHashRetrieved ||
